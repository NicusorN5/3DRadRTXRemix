...
:: Find the MSVC toolchain. ProgramFiles(x86) is not exported by every shell, so it
:: is normalised first; vswhere then locates whichever edition has the C++ tools.
set "PF86=%ProgramFiles(x86)%"
if not defined PF86 set "PF86=%SystemDrive%\Program Files (x86)"
...
if not defined VCVARS (
    echo ERROR: no Visual Studio install with the C++ x86 toolset was found
    exit /b 1
)
...
:: Optional Win32 launch-dialog resource (3D Rad editor)
set "RESFILE="
if exist "%COMP_DIR%\launch_dialog.rc" (
    echo       launch_dialog.rc
    rc /nologo /i"%COMP_DIR%" /fo"%GAME_OBJ%\launch_dialog.res" "%COMP_DIR%\launch_dialog.rc"
    if errorlevel 1 goto :fail
    set "RESFILE=%GAME_OBJ%\launch_dialog.res"
)
...
link /nologo /DLL /SUBSYSTEM:WINDOWS /DEBUG /PDBCompress %LF% %LIBPATH% ^
    ...
    "%OUT%\imgui.lib" ^
    "%OUT%\minhook.lib" ^
    d3dx9.lib psapi.lib user32.lib gdi32.lib gdiplus.lib comctl32.lib uxtheme.lib dwmapi.lib msimg32.lib shell32.lib advapi32.lib ole32.lib winhttp.lib
if errorlevel 1 goto :fail
...
if exist "C:\3DRadRTX\" (
    copy /y "%GAME_OUT%\d3d9.dll" "C:\3DRadRTX\d3d9.dll" >nul
    echo Deployed: C:\3DRadRTX\d3d9.dll
)
exit /b 0
