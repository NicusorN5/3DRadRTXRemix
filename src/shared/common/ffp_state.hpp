...
namespace shared::common
{
	...
	public:
		...
		void on_set_render_state(D3DRENDERSTATETYPE state, DWORD value);
		void on_present(IDirect3DDevice9* dev);
		void on_begin_scene();
		void on_reset();

		// --- Transform input (this game supplies matrices directly) ---

		/*
		 * 3Impact only ever uploads concatenated matrices to its shaders, so the
		 * register-slicing path the framework uses for other games cannot recover a
		 * camera here. comp::game::camera pulls View and Projection out of the engine
		 * and feeds them in through these setters instead.
		 */
		void set_camera(const D3DMATRIX& view, const D3DMATRIX& proj);
		void set_world(const D3DMATRIX& world);

		// PointLight / SunLight captured this frame (0 = use fallback sun).
		static constexpr int k_max_d3d_lights = 8;
		void set_scene_lights(const D3DLIGHT9* lights, int count);
		int scene_light_count() const { return scene_light_count_; }

		// Push the captured View/Projection onto the device. Called as soon as
		// the first good scene camera exists this frame (and again at Present
		// if the hash changed). Remix first-write-wins MAIN from this state.
		// Never overwrite those matrices with identity after Present.
		void apply_pending_camera(IDirect3DDevice9* dev);

		// --- Draw routing tally ---

		/*
		 * Why each draw of the current frame went down the FFP or the passthrough path.
		 * Counted so diagnostics can show whether conversion is actually happening,
		 * rather than silently rendering every mesh with the game's own shaders.
		 */
		enum class draw_route
		{
			ffp,               // converted: transforms recovered, shaders off
			ffp_disabled,      // [FFP] Enabled=0 in the INI
			no_camera,         // no View/Projection captured from the engine yet
			skinned,           // bone palette drives the vertices; FFP cannot do that
			no_normal,         // no NORMAL in the declaration, so treated as HUD/UI
			world_unresolved,  // the constant block was not the expected WVP
			secondary_target,  // cubemap / preview RT, not the editor viewport
			hud,               // D3DFVF_XYZRHW / POSITIONT only — never scene meshes
			pre_game,          // compiled picker/loading: shader path, no 3D camera
			count
		};

		void record_route(draw_route route);
		UINT route_count(draw_route route) const;
		...
		DWORD rs_zwrite() const { return rs_zwrite_; }
		DWORD rs_zenable() const { return rs_zenable_; }
		DWORD rs_blend() const { return rs_blend_; }
		...
		const D3DMATRIX& view() const { return view_; }
		const D3DMATRIX& proj() const { return proj_; }
		const D3DMATRIX& world() const { return world_; }
	...
	private:
		...
		// Transforms supplied by comp::game::camera
		D3DMATRIX view_ = {};
		D3DMATRIX proj_ = {};
		D3DMATRIX world_ = {};
		...
		UINT route_counts_[static_cast<int>(draw_route::count)] = {};

		DWORD rs_zwrite_ = TRUE;
		DWORD rs_zenable_ = TRUE;
		DWORD rs_blend_ = FALSE;

		// Cached config
		const config::ffp_settings* cfg_ = nullptr;

		D3DLIGHT9 scene_lights_[k_max_d3d_lights] = {};
		int scene_light_count_ = 0;
		int last_enabled_lights_ = 0;
