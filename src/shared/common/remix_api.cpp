...
namespace shared::common
{
	...
	namespace
	{
		bool call_set_config_variable(PFN_remixapi_SetConfigVariable pfn,
			const char* key, const char* value, remixapi_ErrorCode* status)
		{
			__try
			{
				*status = pfn(key, value);
				return true;
			}
		...
		bool call_initialize_library(PFN_remixapi_InitializeLibrary pfn,
	...
	bool remix_api::set_config_variable(const char* key, const char* value)
	{
		auto& api = get();
		if (shared::globals::skip_remix ||
			!api.m_initialized || !api.m_bridge.SetConfigVariable || !key || !value)
		{
			return false;
		}

		remixapi_ErrorCode status = REMIXAPI_ERROR_CODE_GENERAL_FAILURE;
		if (!call_set_config_variable(api.m_bridge.SetConfigVariable, key, value, &status))
		{
			...
			if (!logged_fault)
			{
				...
				shared::common::log("RemixApi",
					"SetConfigVariable faulted — ignored (no abort)",
	...
	void remix_api::set_pregame_raster(const bool pregame)
	{
		auto& api = get();
		api.m_pregame_raster = pregame;
		api.m_pregame_raster_applied = false;
	}

	void remix_api::try_initialize(const char* why)
	{
		if (shared::globals::skip_remix) {
			return;
		}
		...
		{
			api.m_init_retries++;
			const bool device_event = why && (std::strcmp(why, "CreateDevice") == 0 ||
				std::strcmp(why, "Reset") == 0);
			const bool periodic = why && (api.m_init_retries % 30) == 0;
			if (device_event || periodic)
			{
				shared::common::log("RemixApi",
					std::format("Retry after {}: still Code {}{}",
						why, api.m_last_init_status,
						api.m_last_init_status == REMIXAPI_ERROR_CODE_NOT_INITIALIZED
							? " (Code 11: .trex/bridge.conf exposeRemixApi is False — not a missing device)"
							: ""),
					shared::common::LOG_TYPE::LOG_TYPE_WARN, true);
			}
			return;
		...
		if (why && api.m_init_retries > 0)
		{
			shared::common::log("RemixApi",
				std::format("Initialized after {} ({} retr{})",
					why, api.m_init_retries, api.m_init_retries == 1 ? "y" : "ies"),
				shared::common::LOG_TYPE::LOG_TYPE_STATUS, true);
			api.m_init_retries = 0;
		}
	}

	bool remix_api::setup_world_camera(const D3DMATRIX& view, const D3DMATRIX& proj,
		const float* eye_xyz, const bool have_proj_params,
		const float fov_y_deg, const float aspect, const float z_near, const float z_far)
	{
		auto& api = get();
		if (!api.m_initialized)
		{
		...
		if (!api.m_initialized || !api.m_bridge.SetupCamera)
		{
			if (api.m_initialized && !api.m_logged_no_setup_camera)
			{
				api.m_logged_no_setup_camera = true;
				shared::common::log("RemixApi",
					"SetupCamera is null on this bridge (GMod 32-bit client leaves it unset). "
					"Camera goes via early SetTransform so Remix still first-write-wins this frame.",
					shared::common::LOG_TYPE::LOG_TYPE_WARN, true);
			}
			return false;
		}
		...
		for (int row = 0; row < 4; row++)
		{
			...
			{
				info.view[row][col] = view.m[row][col];
				info.projection[row][col] = proj.m[row][col];
			}
		}

		if (have_proj_params && eye_xyz &&
			std::isfinite(fov_y_deg) && fov_y_deg > 1.0f && fov_y_deg < 179.0f &&
			std::isfinite(aspect) && aspect > 0.05f &&
			std::isfinite(z_near) && z_near > 0.0f &&
			std::isfinite(z_far) && z_far > z_near)
		{
			auto axis = [](const float x, const float y, const float z, const remixapi_Float3D fallback)
				{
					const float length = std::sqrt(x * x + y * y + z * z);
					if (!(length > 1e-4f) || !std::isfinite(length)) {
						return fallback;
					}
					return remixapi_Float3D{ x / length, y / length, z / length };
				};

			params.sType = REMIXAPI_STRUCT_TYPE_CAMERA_INFO_PARAMETERIZED_EXT;
			params.pNext = nullptr;
			params.position = { eye_xyz[0], eye_xyz[1], eye_xyz[2] };
			params.right = axis(view._11, view._21, view._31, { 1.0f, 0.0f, 0.0f });
			params.up = axis(view._12, view._22, view._32, { 0.0f, 1.0f, 0.0f });
			params.forward = axis(view._13, view._23, view._33, { 0.0f, 0.0f, 1.0f });
			params.fovYInDegrees = fov_y_deg;
			params.aspect = aspect;
			params.nearPlane = z_near;
			params.farPlane = z_far;
			info.pNext = &params;
		}

		const auto status = api.m_bridge.SetupCamera(&info);
		if (status != REMIXAPI_ERROR_CODE_SUCCESS)
		{
			...
			if (logged < 4)
			{
				logged++;
				shared::common::log("RemixApi",
					std::format("SetupCamera failed - Code: {:d}", static_cast<int>(status)),
					shared::common::LOG_TYPE::LOG_TYPE_WARN, true);
			}
			return false;
		}
		...
		bool /*is_asi*/)
	{
		...
		if (shared::globals::skip_remix) return;
		...
		char remix_path[MAX_PATH]{};
		GetModuleFileNameA(remix_module, remix_path, MAX_PATH);

		auto pfn_init = (PFN_remixapi_InitializeLibrary)
			GetProcAddress(remix_module, remixapi::exported_func_name::initRemixApi);
		if (!pfn_init)
		{
			if (!instance.m_logged_init_status)
			{
				instance.m_logged_init_status = true;
				shared::common::log("RemixApi",
					std::format("Remix bridge does not export remixapi_InitializeLibrary ({})",
						remix_path[0] ? remix_path : "no path"),
		...
		remixapi_Interface iface{};
		remixapi_ErrorCode status = REMIXAPI_ERROR_CODE_GENERAL_FAILURE;
		if (!call_initialize_library(pfn_init, &init_info, &iface, &status))
		{
			if (!instance.m_logged_init_status)
			{
				instance.m_logged_init_status = true;
				shared::common::log("RemixApi",
					std::format("InitializeLibrary faulted from {} — not aborting",
						remix_path[0] ? remix_path : "d3d9_chain"),
					shared::common::LOG_TYPE::LOG_TYPE_ERROR, true);
			}
			return;
		}
		instance.m_last_init_status = static_cast<int>(status);

		if (status != REMIXAPI_ERROR_CODE_SUCCESS)
		{
			// Code 11 = NOT_INITIALIZED. On this bridge that is almost always
			// exposeRemixApi=False in .trex/bridge.conf (logged in bridge32.log).
			// A missing D3D device is not what this export returns.
			if (!instance.m_logged_init_status)
			{
				instance.m_logged_init_status = true;
				const char* hint = (status == REMIXAPI_ERROR_CODE_NOT_INITIALIZED)
					? " (Code 11: exposeRemixApi is False in .trex/bridge.conf — proxy writes True before LoadLibrary)"
					: "";
				shared::common::log("RemixApi",
					std::format("Failed to initialize the remixApi - Code: {:d} from {}{}",
						static_cast<int>(status),
						remix_path[0] ? remix_path : "d3d9_chain", hint),
					shared::common::LOG_TYPE::LOG_TYPE_ERROR, true);
			}
		...
		shared::common::log("RemixApi",
			std::format("Initialized RemixApi (SetupCamera={})",
				iface.SetupCamera ? "yes" : "null — early SetTransform"),
			shared::common::LOG_TYPE::LOG_TYPE_STATUS, true);
	}
}
