...
namespace shared::common
{
	...
	// Compiled player: keep the log console, but do not let AllocConsole own
	// the taskbar or stay the foreground window. WS_EX_TOOLWINDOW hides the
	// button; SHOWNOACTIVATE avoids stealing the game hwnd.
	inline void demote_debug_console()
	{
    ...
    inline void console()
    {
        if (!g_external_console_created)
        {
			...
			if (shared::globals::is_compiled_host && !shared::globals::is_editor_host) {
				demote_debug_console();
			}
        }
    }
	...
	inline void init_log_file()
	{
		std::call_once(log_file_once, []() {
			const std::string dir_path = shared::globals::host_data_root() + "\\rtx_comp";
	...
	inline void log(const std::string_view& module_str, const std::string_view& msg, LOG_TYPE type = LOG_TYPE::LOG_TYPE_DEFAULT, bool highlight = false, bool newline_infront = false)
	{
		...
		if (log_file.is_open())
		{
			...
			// Flushing every Present hitching the game. Errors/warnings still
			// land immediately; everything else batches.
			static unsigned log_lines = 0;
			if (type == LOG_TYPE::LOG_TYPE_ERROR || type == LOG_TYPE::LOG_TYPE_WARN ||
				(++log_lines & 31u) == 0)
			{
				log_file.flush();
			}
