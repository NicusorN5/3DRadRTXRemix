...
namespace comp
{
	...
	void main()
	{
		// Editor finds 3DRADCLASS before CreateDevice; the compiled player never
		// creates that class, so this also runs from CreateDevice. Once only —
		// double register would construct imgui/renderer twice.
		static volatile long started = 0;
		if (InterlockedCompareExchange(&started, 1, 0) != 0) {
			return;
		}

		// Remix InitializeLibrary is Code 11 (NOT_INITIALIZED) if it runs before
		// CreateDevice. Do not init here; d3d9ex retries after the device exists.
