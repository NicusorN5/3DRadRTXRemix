...
namespace d3d9_proxy
{
	...
	// Compiled player held d3d9_remix.dll until Display Options wrote conf
	// and the first real game hwnd CreateDevice. Editor never defers.
	bool remix_deferred();

	// Late-load Remix for compiled play after Display Options OK.
	// Returns remix IDirect3D9 when hwnd is valid (not 0, not 10x10 dummy)
	// and the picker has been dismissed. nullptr = use system IDirect3D9.
