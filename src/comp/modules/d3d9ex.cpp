...
#include "comp/game/particles.hpp"
#include "comp/game/fog.hpp"
...
#pragma region D3D9Device
	...
	HRESULT d3d9ex::D3D9Device::GetDirect3D(IDirect3D9** ppD3D9)
	{
		...
		// Return the proxy IDirect3D9, not Remix's inner interface. A second
		// CreateDevice on the unwrapped object skipped our logs and AVed the bridge.
		if (auto* wrap = shared::globals::d3d9_interface)
		{
			wrap->AddRef();
			*ppD3D9 = wrap;
			return D3D_OK;
	...
	HRESULT d3d9ex::D3D9Device::Reset(D3DPRESENT_PARAMETERS* pPresentationParameters)
	{
		...
		shared::common::log("d3d9", std::format("Reset device={:p} {}x{} windowed={}",
			static_cast<void*>(m_pIDirect3DDevice9),
		...
		renderer::invalidate_render_target_cache();
		...
		// Destroy Remix particle mesh/material while the device is still
		// alive. CreateLight-style recreate after Reset AVed NvRemixBridge.
		game::particles::prepare_reset();
		game::fog::reset();
		const bool imgui_dx = imgui::get() && imgui::get()->m_initialized_device;
		if (imgui_dx) {
		...
		if (SUCCEEDED(hr))
		{
			const UINT w = pPresentationParameters ? pPresentationParameters->BackBufferWidth : 0;
			const UINT h = pPresentationParameters ? pPresentationParameters->BackBufferHeight : 0;
			const HWND hwnd = pPresentationParameters ? pPresentationParameters->hDeviceWindow : nullptr;
			const BOOL windowed = pPresentationParameters ? pPresentationParameters->Windowed : TRUE;
			game::camera::on_device_reset(m_pIDirect3DDevice9, w, h, hwnd, windowed);
			main();
			if (game::camera::remix_api_init_allowed()) {
				shared::common::remix_api::try_initialize("Reset");
			}
			if (hwnd)
			{
				RECT client{};
	...
	HRESULT d3d9ex::D3D9Device::Present(CONST RECT* pSourceRect, CONST RECT* pDestRect, HWND hDestWindowOverride, CONST RGNDATA* pDirtyRegion)
	{
		...
		game::camera::on_present();
		ffp.apply_pending_camera(m_pIDirect3DDevice9);
		if (auto* d = diagnostics::get()) d->on_present(ffp.frame_count(), ffp.draw_call_count(), ffp.scene_count());
		ffp.on_present(m_pIDirect3DDevice9);
	...
	HRESULT d3d9ex::D3D9Device::SetRenderTarget(DWORD RenderTargetIndex, IDirect3DSurface9* pRenderTarget)
	{
		...
		const HRESULT hr = m_pIDirect3DDevice9->SetRenderTarget(RenderTargetIndex, pRenderTarget);
		if (SUCCEEDED(hr)) {
			renderer::note_render_target(m_pIDirect3DDevice9, RenderTargetIndex, pRenderTarget);
		}
		return hr;
	}
	...
	HRESULT d3d9ex::D3D9Device::SetViewport(CONST D3DVIEWPORT9* pViewport)
	{
		...
		if (pViewport)
		{
			// Remix MAIN: editor 3D RT once known; compiled player backbuffer.
			// Leftover windowed/client rects (1519×824 on 1920×1080) are pinned
			// so they cannot rescale HUD. Editor 1332 is never pinned to 827.
			D3DVIEWPORT9 viewport = *pViewport;
			const bool pinned = game::camera::adjust_pregame_viewport(
				m_pIDirect3DDevice9, viewport);
			HRESULT hr = pinned
				? m_pIDirect3DDevice9->SetViewport(&viewport)
				: m_pIDirect3DDevice9->SetViewport(pViewport);
			if (FAILED(hr) && pinned) {
				hr = m_pIDirect3DDevice9->SetViewport(pViewport);
			}
			if (SUCCEEDED(hr))
			{
				const UINT w = pinned ? viewport.Width : pViewport->Width;
				const UINT h = pinned ? viewport.Height : pViewport->Height;
				game::camera::on_viewport(w, h);
			}
			return hr;
		}
	...
	HRESULT d3d9ex::D3D9Device::SetRenderState(D3DRENDERSTATETYPE State, DWORD Value)
	{
		...
		shared::common::ffp_state::get().on_set_render_state(State, Value);
	...
	HRESULT d3d9ex::D3D9Device::SetTexture(DWORD Stage, IDirect3DBaseTexture9* pTexture)
	{
		...
		editor_settings::note_texture(Stage, pTexture);
	...
	HRESULT d3d9ex::D3D9Device::DrawPrimitive([[maybe_unused]] D3DPRIMITIVETYPE PrimitiveType, [[maybe_unused]] UINT StartVertex, [[maybe_unused]] UINT PrimitiveCount)
	{
		...
		if (auto* r = renderer::get()) {
			return r->on_draw_primitive(m_pIDirect3DDevice9, PrimitiveType, StartVertex, PrimitiveCount);
		}
	...
	HRESULT d3d9ex::D3D9Device::DrawIndexedPrimitive(D3DPRIMITIVETYPE PrimitiveType, INT BaseVertexIndex, UINT MinVertexIndex, UINT NumVertices, UINT startIndex, UINT primCount)
	{
		...
		if (auto* r = renderer::get()) {
			return r->on_draw_indexed_prim(m_pIDirect3DDevice9, PrimitiveType, BaseVertexIndex, MinVertexIndex, NumVertices, startIndex, primCount);
		}
...
#pragma region _D3D9
	...
	HRESULT __stdcall d3d9ex::_d3d9::CreateDevice(UINT Adapter, D3DDEVTYPE DeviceType, HWND hFocusWindow, DWORD BehaviorFlags, D3DPRESENT_PARAMETERS* pPresentationParameters, IDirect3DDevice9** ppReturnedDeviceInterface)
	{
		...
		char device_cls[64]{};
		if (hFocusWindow) {
			GetClassNameA(hFocusWindow, focus_cls, sizeof(focus_cls));
		}
		const HWND device_hwnd = (pPresentationParameters && pPresentationParameters->hDeviceWindow)
			? pPresentationParameters->hDeviceWindow : hFocusWindow;
		if (device_hwnd && device_hwnd != hFocusWindow) {
			GetClassNameA(device_hwnd, device_cls, sizeof(device_cls));
		}

		const UINT bb_w = pPresentationParameters ? pPresentationParameters->BackBufferWidth : 0;
		const UINT bb_h = pPresentationParameters ? pPresentationParameters->BackBufferHeight : 0;
		IDirect3D9* remix = d3d9_proxy::remix_d3d_for_hwnd(
			D3D_SDK_VERSION, hFocusWindow, device_hwnd, bb_w, bb_h);
		IDirect3D9* impl = remix ? remix : m_pIDirect3D9;
		HRESULT hres = impl->CreateDevice(Adapter, DeviceType, hFocusWindow, BehaviorFlags, pPresentationParameters, ppReturnedDeviceInterface);
		if (remix && FAILED(hres))
		{
			shared::common::log("d3d9",
				std::format("Remix CreateDevice failed hr=0x{:X} — falling back to system d3d9",
					static_cast<unsigned>(hres)),
				shared::common::LOG_TYPE::LOG_TYPE_WARN, true);
			hres = m_pIDirect3D9->CreateDevice(Adapter, DeviceType, hFocusWindow, BehaviorFlags, pPresentationParameters, ppReturnedDeviceInterface);
			remix = nullptr;
		}

		// 3D Rad creates more than one device, so identify them: the FFP path tracks
		// state in one place and needs to know which device a draw belongs to.
		shared::common::log("d3d9", std::format(
			"CreateDevice hwnd=0x{:X} class={} device_hwnd=0x{:X} class={} flags=0x{:X} {}x{} windowed={} via={} -> device={:p} hr=0x{:X}",
			reinterpret_cast<std::uintptr_t>(hFocusWindow), focus_cls[0] ? focus_cls : "?",
			reinterpret_cast<std::uintptr_t>(device_hwnd),
			device_cls[0] ? device_cls : (focus_cls[0] ? focus_cls : "?"),
			BehaviorFlags,
			pPresentationParameters ? pPresentationParameters->BackBufferWidth : 0,
			pPresentationParameters ? pPresentationParameters->BackBufferHeight : 0,
			pPresentationParameters ? pPresentationParameters->Windowed : 0,
			remix ? "remix" : "system",
			static_cast<void*>(ppReturnedDeviceInterface ? *ppReturnedDeviceInterface : nullptr),
			static_cast<unsigned>(hres)));

		if (hFocusWindow && !shared::globals::main_window) {
			shared::globals::main_window = hFocusWindow;
		}
		...
		if (SUCCEEDED(hres) && *ppReturnedDeviceInterface) {
			*ppReturnedDeviceInterface = new d3d9ex::D3D9Device(*ppReturnedDeviceInterface);
			...
			const UINT w = pPresentationParameters ? pPresentationParameters->BackBufferWidth : 0;
			const UINT h = pPresentationParameters ? pPresentationParameters->BackBufferHeight : 0;
			HWND created_hwnd = hFocusWindow;
			if (pPresentationParameters && pPresentationParameters->hDeviceWindow) {
				created_hwnd = pPresentationParameters->hDeviceWindow;
			}
			const BOOL windowed = pPresentationParameters ? pPresentationParameters->Windowed : TRUE;
			game::camera::on_device_created(created_hwnd, w, h, windowed);
			if (hFocusWindow && hFocusWindow != created_hwnd) {
				game::camera::on_device_created(hFocusWindow, w, h, windowed);
			}
			editor_frame::note_create_device();

			// Compiled player CreateDevice #1 is the 1080 windowed boot device
			// that is destroyed when the picker applies 1920. Editor also
			// CreateDevice then Reset/recreates. InitializeLibrary on those
			// throwaway devices AVed NvRemixBridge. Wait until a scene camera
			// exists (remix_api_init_allowed).
			main();
			if (game::camera::remix_api_init_allowed()) {
				shared::common::remix_api::try_initialize("CreateDevice");
			}
		}
...
#pragma region _D3D9Ex
	...
	HRESULT __stdcall d3d9ex::_d3d9ex::CreateDevice(UINT Adapter, D3DDEVTYPE DeviceType, HWND hFocusWindow, DWORD BehaviorFlags, D3DPRESENT_PARAMETERS* pPresentationParameters, IDirect3DDevice9** ppReturnedDeviceInterface)
	{
		...
		IDirect3D9* remix = d3d9_proxy::remix_d3d_for_hwnd(
			D3D_SDK_VERSION, hFocusWindow, device_hwnd, bb_w, bb_h);
		HRESULT hres = remix
			? remix->CreateDevice(Adapter, DeviceType, hFocusWindow, BehaviorFlags, pPresentationParameters, ppReturnedDeviceInterface)
			: m_pIDirect3D9Ex->CreateDevice(Adapter, DeviceType, hFocusWindow, BehaviorFlags, pPresentationParameters, ppReturnedDeviceInterface);
		if (remix && FAILED(hres))
		{
			shared::common::log("d3d9",
				std::format("Remix CreateDevice (ex path) failed hr=0x{:X} — falling back to system",
					static_cast<unsigned>(hres)),
				shared::common::LOG_TYPE::LOG_TYPE_WARN, true);
			hres = m_pIDirect3D9Ex->CreateDevice(Adapter, DeviceType, hFocusWindow, BehaviorFlags, pPresentationParameters, ppReturnedDeviceInterface);
			remix = nullptr;
		}
		...
		if (SUCCEEDED(hres) && *ppReturnedDeviceInterface) {
			*ppReturnedDeviceInterface = new d3d9ex::D3D9Device(*ppReturnedDeviceInterface);
			...
			{
				const HWND created_hwnd = pPresentationParameters->hDeviceWindow
					? pPresentationParameters->hDeviceWindow : hFocusWindow;
				game::camera::on_device_created(created_hwnd,
...
{
	comp::launch::ensure_ready();
	if (comp::launch::aborted()) {
		ExitProcess(0);
	}
	...
	if (shared::globals::d3d_passthrough())
	{
		shared::common::log("d3d9", "Direct3DCreate9 passthrough (no Remix, no FFP wrapper).");
		shared::globals::d3d9_interface = real_d3d9;
		return real_d3d9;
	}

	shared::common::log("d3d9", "Direct3DCreate9 called. Creating proxy interface.");

	// dll3impact.dll imported us, so its IAT is resolved. Hook PerspectiveFov /
	// LookAt / MultiplyTranspose before the engine builds scene cameras.
	comp::game::camera::init();
...
extern "C" HRESULT WINAPI Direct3DCreate9Ex(UINT SDKVersion, IDirect3D9Ex** ppD3D)
{
	...
	if (shared::globals::d3d_passthrough())
	{
		shared::common::log("d3d9", "Direct3DCreate9Ex passthrough (no Remix, no FFP wrapper).");
		shared::globals::d3d9_interface = real_d3d9ex;
		*ppD3D = real_d3d9ex;
		return hr;
	}
