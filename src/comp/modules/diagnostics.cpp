...
namespace comp
{
	...
	void diagnostics::on_present(UINT frame_count, UINT draw_count, UINT scene_count)
	{
		...
		if (cat_present_info)
		{
			...
			// How the frame's draws were routed, and the transforms the FFP path used.
			// A frame where everything lands outside 'ffp' is rendering with the game's
			// own shaders, which Remix cannot replace.
			using route = shared::common::ffp_state::draw_route;
			log_str("  Draw routing:\r\n");
			log_int("    ffp=", static_cast<int>(ffp.route_count(route::ffp)));
			log_int("    ffpDisabled=", static_cast<int>(ffp.route_count(route::ffp_disabled)));
			log_int("    noCamera=", static_cast<int>(ffp.route_count(route::no_camera)));
			log_int("    skinned=", static_cast<int>(ffp.route_count(route::skinned)));
			log_int("    noNormal=", static_cast<int>(ffp.route_count(route::no_normal)));
			log_int("    worldUnresolved=", static_cast<int>(ffp.route_count(route::world_unresolved)));
			log_int("    secondaryTarget=", static_cast<int>(ffp.route_count(route::secondary_target)));
			log_int("    hud=", static_cast<int>(ffp.route_count(route::hud)));
			log_int("    preGame=", static_cast<int>(ffp.route_count(route::pre_game)));

			if (ffp.view_proj_valid())
			{
				log_matrix("  View", &ffp.view()._11);
				log_matrix("  Proj", &ffp.proj()._11);
				log_matrix("  World (last)", &ffp.world()._11);
			}
