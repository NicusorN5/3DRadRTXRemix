...
#undef ADD_TAB
	...
	void imgui::on_present()
	{
		if (auto* im = imgui::get(); im)
		{
			...
			{
				if (game::camera::is_compiled_player() &&
					shared::globals::main_window &&
					im->m_initialized)
				{
					static HWND bound = nullptr;
					if (!bound) {
						bound = shared::globals::main_window;
					}
					if (bound && bound != shared::globals::main_window)
					{
						if (g_game_wndproc && bound)
						{
							SetWindowLongPtr(bound, GWLP_WNDPROC,
								reinterpret_cast<LONG_PTR>(g_game_wndproc));
							g_game_wndproc = nullptr;
						}
						ImGui_ImplWin32_Shutdown();
						ImGui_ImplWin32_Init(shared::globals::main_window);
						g_game_wndproc = reinterpret_cast<WNDPROC>(SetWindowLongPtr(
							shared::globals::main_window, GWLP_WNDPROC, LONG_PTR(wnd_proc_hk)));
						char cls[64]{};
						GetClassNameA(shared::globals::main_window, cls, sizeof(cls));
						shared::common::log("ImGui", std::format(
							"rebound hwnd=0x{:X} class={}",
